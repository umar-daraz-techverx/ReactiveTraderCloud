...
const addListener = () => {
	if (isFinsemble) {
		return FSBL.Clients.RouterClient.addListener(TOPIC, (err, {data: {symbol, interval}}) => configureChart(interval, symbol)) 
	} 
	if (isOpenFin) {
		return fin.desktop.InterApplicationBus.subscribe('*', TOPIC, ({symbol, interval}) => configureChart(interval, symbol));
	}
}
